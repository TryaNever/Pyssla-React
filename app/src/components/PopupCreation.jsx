    ...
    return (
        <div className={`fixed top-0 w-screen h-screen bg-black/50 text-white`}>
            <form
                ...
                <i className="ri-close-circle-line text-white text-4xl absolute top-5 right-8" onClick={onClose}></i>

                <div className={`flex items-center gap-5`}>
                    <label htmlFor="nameCanva">Nom du dessin :</label>
                    <input type="text" name="nameCanva" id="nameCanva" placeholder={`Nom de votre dessin`}
                           maxLength={30} minLength={2} className={`outline outline-white rounded-lg px-3 py-1`}/>
                </div>
                <div>
                    <div>
                        <label htmlFor="SizeCanvaX">Largeur du dessin</label>
                        <input type="range" name="SizeCanvaX" id="SizeCanvaX" max={512} min={8} value={sizeX}
                               onChange={(e) => setSizeX(Number(e.target.value))}/>
                    </div>
                    <div>
                        <label htmlFor="SizeCanvaY">Hauteur du dessin</label>
                        <input type="range" name="SizeCanvaY" id="SizeCanvaX" max={512} min={8} value={sizeY}
                ...
                <div className={`text-black`}>
                    <input type="radio" name="typeShape" id="circle" defaultChecked={true}
                           className={`peer/circle hidden`}/>
